...
var Packet = function (data, num, type, size) {
  ...
  this.buff[0] = MAGICA;
  this.buff[1] = MAGICB;
  this.buff.writeUInt16BE(size, 2); // we need or not?
  this.buff[6] = num;
  this.buff[7] = type;
};
...
Protocol.prototype.checkPacket = function (data) {
  ...
  if (data[6] === this.counters[RXT]) {
    if (data[7] === DataTypes.DATA){
      this.counters[RX]++;
    ...
    if (data[6] === DataTypes.SYSTEM){
      if (data[7] === MessageTypes.REQUEST){
        var num = data[7];
        this.emit('packet', this.packets[num].buff);
      }
      return true;
    }
    if (data[6] === DataTypes.USER) {
